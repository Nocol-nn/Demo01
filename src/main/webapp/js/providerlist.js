...
function deleteProvider(obj) {
    $.ajax({
        type: "DELETE",
        url: path + "/provider/delete/" + obj.attr("proid"),
        ...
        success: function (data) {
            if (data.delResult == "true") {//删除成功：移除删除行
                ...
                location.href = path + "/provider/search";
...
$(function () {
    ...
    //     //将被绑定的元素（a）转换成jquery对象，可以使用jquery方法
    //     var obj = $(this);
    //     window.location.href = path + "/provider/remove/" + obj.attr("proid");
    // });

    $(".removeProvider").on("click", function () {
        var obj = $(this);
        if (confirm("你确定要移除供应商【" + obj.attr("proname") + "】吗？")) {
            window.location.href = path + "/provider/remove/" + obj.attr("proid");
        }
        // changeDLGContent("你确定要移除供应商【" + providerObj.attr("proname") + "】吗？");
        // openYesOrNoDLG();
    });
