...
$(function () {
    ...
    proPhone.next().html("*");
    provincialId.next().html("*");


    //加载所有省份
    $.ajax({
        type: "GET",//请求类型
        url: path + "/provincial/getAllProvincial",//请求的url
        data: {},//请求参数
        dataType: "json",//ajax接口（请求url）返回的数据类型
        success: function (data) {//data：返回数据（json对象）
            if (data != null) {
                // var provincialId = $("#provincialId").val();
                provincialId.html("");
                ...
                if (data != null) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        options += "<option value=\"" + data[i].id + "\">" + data[i].provincialName + "</option>";
                    }
                    provincialId.html(options);
                }
                provincialId.html(options);
            }
        },
        error: function (data) {//当访问时候，404，500 等非200的错误状态码
            validateTip(provincialId.next(), {"color": "red"}, imgNo + " 获取用户角色列表error", false);
        }
    });
    ...
    proName.on("focus", function () {
        ...
        $.ajax({
            type: "GET",
            url: path + "/provider/isExists",
            data: {"proName": encodeURIComponent(proName.val())},
            dataType: "json",//ajax接口（请求url）返回的数据类型
            success: function (data) {
                if (data.delResult == "true") { //不存在
                    validateTip(proName.next(), {"color": "green"}, imgYes, true);
                } else if (data.delResult == "false") {//存在
                    validateTip(proName.next(), {"color": "red"}, imgNo + " 供应商名称已存在", false);
                }
            },
            error: function (data) {
                //请求出错
                validateTip(proName.next(), {"color": "red"}, imgNo + " 请求错误", false);
            }
        });
    });
